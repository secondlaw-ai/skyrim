...
def test_format_to_datetime():
    time_list = CDS(channels=["u10m"])._ymdt_to_datetime(2023, 1, [1, 2], 0)
    assert time_list == [
...
def test_batch_timestamps_daily():
    timestamps = [
        datetime.datetime(2023, 1, 1, 0, 0),
    ...
    cds = CDS(channels=["u10m"])
    assert cds._batch_timestamps_daily(timestamps) == expected


def test_batch_timestamps_monthly():
    timestamps = [
        datetime.datetime(2023, 1, 1, 0, 0),
    ...
    cds = CDS(channels=["u10m"])
    result = cds._batch_timestamps_monthly(timestamps)
